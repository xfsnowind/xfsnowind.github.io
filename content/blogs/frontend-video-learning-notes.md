...
The difference of video streaming from live streaming is live streaming sending the live content generated by user's camera instead of fixed the video files. And normally live streaming has higher requirement for casting the live video to multiple users simultaneously. So the concept simulcasting is the process of streaming the same content to multiple destinations at the same time.
...
Websocket is a realtime protocol that enable client-server bi-directional communication over a persisitent, single-socket connection.
